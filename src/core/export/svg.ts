...
/**
 * Build a standalone SVG string for square or dot modes (relief falls back to
 * flat squares). Vector output scales crisply to any size.
 */
export function buildSvg(sample: SampledGrid, req: RenderRequest): string {
  ...
  const cells = req.renderMode === "dot" ? dotCells(sample, req, cellSize) : squareCells(sample, req, cellSize);
